...
type BookRepo interface {
	GetAll(ctx context.Context, filter forms.BookFilter) ([]data.Book, error)
	GetById(ctx context.Context, id int) (data.Book, error)
	GetByCategoryID(ctx context.Context, id int, filter forms.BookFilter) ([]data.Book, error)
...
func (repo *bookRepo) GetAll(ctx context.Context, filter forms.BookFilter ) ([]data.Book, error) {
	var books []data.Book
	db, err := config.MySQL()
	...
	query := fmt.Sprintf("SELECT * FROM %v WHERE 1=1", bookTable)
	queryText := utils.BookFilter(query, filter)
...
func (repo *bookRepo) GetByCategoryID(ctx context.Context, id int, filter forms.BookFilter) ([]data.Book, error) {
	var books []data.Book
	db, err := config.MySQL()
	...
	query := fmt.Sprintf("SELECT * FROM %v WHERE category_id = %v ", bookTable, id)
	queryText := utils.BookFilter(query, filter)
...
func (repo *bookRepo) Insert(ctx context.Context, book forms.InsertBook) error {
	...
	queryText := fmt.Sprintf("INSERT INTO %s (title, description, image_url, release_year, price, total_page, thickness, created_at, updated_at, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), ?)",
		bookTable,
	)
